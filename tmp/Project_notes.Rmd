...
The dataset provide as part of environment reporting legislation requirements in pdf format and has been transposed to an excel. Detailed information about the program can be found [here](https://www2.gov.bc.ca/gov/content/environment/waste-management/recycling/product-stewardship).
...
Broadly, each of the above indicators has data on 
1) quantity of items (kgs/litres) per regional district, 
2) financial summaries; and 
3) units moved

Financial and unit data is compiled at the provincial scale (not regional).

Not all indicators had available data on all three catergories and collection years varied per indicator. 

Some changes to regional district names and associations was addressed. This included updating regional district names (Queen Charlotte/Skeena to North Coast, Powell River to Qathet), and combining regional districts (Comox and Strathcona). The northern rockies data was also incorporated.  
...
3) Initially each indicator was explored to detect patterns and possible data errors. This is shown below.

4) Still to do : 
- refine data summaries.
- generate combined summaries (all indicators?)
- pull out key messages.
- explore graphics for data visualisation.


** Note: during this process when typos and errors were found in the original
datasets manual updates were made and highlighted to correct or to be double checked with the data custodian. 
...
```{r loadlibs, echo = TRUE, include = FALSE, warning=FALSE}
library(readr)
library(dplyr)
library(ggplot2)
library(tidyr)
...
```{r loaddata, echo=TRUE, include = FALSE, warning = FALSE}
...
This plot indicates some data anomolies in 2015 forthe central coast and Central Kootneys. After verifying with the original [datasets](https://www2.gov.bc.ca/assets/gov/environment/waste-management/recycling/recycle/beverage-containers/ar/bdl/2015_bdl_annual_report.pdf) these numbers are the same as those reported in the original pdf.
 
```{r weight per cap, echo=FALSE, include = TRUE, warnings = FALSE}
#weight per capita
...
The weight per capital graphs show similar trends. Perhaps some data annomolies as Metro Van. and Peace River patterns are identical. The 2015 North Coast outlier was adjusted in the raw data after checking pdf. 

More interesting is the divergence from the BC average to enable regions to see how they compare to the provincial average (above or below). 
...
```{r, echo= FALSE, include = TRUE, warnings = FALSE}
...
#test <- diff.df %>% dplyr::filter(regional_district == "Comox Strathcona") 
#pop %>% filter(regional_district == "Comox Strathcona") 
...
```
Unsurewhat is going on here? The pattern is not easy to interpret. 
...
Checked the raw data for the Northern Rockies and this is correct (same as report) although the same value (179.47) is given for both Stikine and Northern Rockies for multiple values (oil, containers). Possible error in raw dataset? Values for the peace are high but not too out of the order of possible. 
...
Similar patterns might indcate problem with the population data estimates rather than pattern. 
...
```{r, include = TRUE, echo = FALSE}
tire_units <- all.units %>% filter(type == 'tire')
...
ph_data <- left_join(weight.per.cap, pop,
  ...
  mutate(weight.per.cap = total / pop)
...
bc.weight.per.cap_summary <- ph_data %>%
  na.omit() %>%
  summarise(bc_ave = mean(weight.per.cap))

regional.weight.per.cap_summary <- ph_data %>%
  na.omit() %>%
  group_by(regional_district) %>%
  summarise(ave = mean(weight.per.cap))

# join the regional and prov. ave data and calculate the difference
diff_df_summary <- regional.weight.per.cap_summary %>%
  mutate(delta = ave - bc.weight.per.cap_summary$bc_ave) %>%
  mutate(response = ifelse(delta < 0,"below", "above")) %>%
  mutate(response = ifelse(delta == 0,"No data", response))
  
ggplot(diff_df_summary, aes(x = regional_district,
                    y = delta,
                    label = delta)) +
  geom_bar(stat = 'identity', aes(fill = response), width = .5)  +
  scale_fill_manual(name = "Mileage",
...
# Diverging Barcharts (all years)
bc.weight.per.cap_summary <- ppp_data %>%
  na.omit() %>%
  summarise(bc_ave = mean(weight.per.cap))

regional.weight.per.cap_summary <- ppp_data %>%
  na.omit() %>%
  group_by(regional_district) %>%
  summarise(ave = mean(weight.per.cap))
