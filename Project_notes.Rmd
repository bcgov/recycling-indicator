...
In most cases data was combined with BC population data to calculate per person values per region. BC population data was manualy extracted per regional district for 2000 - 2018 using the [BC Population tool](https://www.bcstats.gov.bc.ca/apps/PopulationEstimates.aspx). The exception to this was Oil, lubricant, antifreeze dataset in which the raw data was already summarised to a per capita measure. The raw data is available within the reports. 
...
Financial and unit data is compiled at the provincial scale.  

Not all indicators had available data on all three catergories and collection years varied per indicator. 

Some changes to regional district names and associations (merging) was also addressed. This included updating regional district names (Queen Charlotte/Skeena to North Coast, Powell River to Qathet, joining Comox and Strathcona into a single region)
...
This plot indicates some data anomolies in the 2015 datasets forthe central coast and Central Kootneys. After verifying with the original [datasets](https://www2.gov.bc.ca/assets/gov/environment/waste-management/recycling/recycle/beverage-containers/ar/bdl/2015_bdl_annual_report.pdf) these numbers are the same as those reported in the original pdf.
 
```{r weight per cap, echo=FALSE, include = TRUE}
#weight per capita
...
The weight per capital graphs show similar trends. Perhaps some data annomolies with the Metro Van. and Peace River patterns are identical (suspicious?). The outlier in for 2015 North Coast was adjusted in the raw data after checking pdf. 
