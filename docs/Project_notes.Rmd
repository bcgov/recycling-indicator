...
1) All raw data is extracted from the excel worksheets and combined into a three dataframes using the ```r clean.readxl.R ``` script. The dataframes are ```r all.finance, all.units, all.regions```

2) Data is then loading into R and inital summaries and exploration is conducted using the ```r 01_load.R ``` script 
